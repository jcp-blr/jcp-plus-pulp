...
@click.command("jcp-plus-pulp-qt", help="A trayicon and service manager for JCP+ PULP")
@click.option(
...
@click.option(
    "--no-gui",
    is_flag=True,
    help="Start jcp-plus-pulp-qt without a graphical user interface (terminal output only)",
)
@click.option(
    "-i",
    "--interactive",
    "interactive_cli",
    is_flag=True,
    help="Start jcp-plus-pulp-qt in interactive cli mode (forces --no-gui)",
)
def main(
    testing: bool,
    verbose: bool,
    ...
    if platform.system() == "Darwin":
        subprocess.call("syslog -s 'jcp-plus-pulp-qt started'", shell=True)

    setup_logging("jcp-plus-pulp-qt", testing=testing, verbose=verbose, log_file=True)
    logger.info("Started jcp-plus-pulp-qt...")

    # Since the .app can crash when started from Finder for unknown reasons, we send a syslog message here to make debugging easier.
    if platform.system() == "Darwin":
        subprocess.call("syslog -s 'jcp-plus-pulp-qt successfully started logging'", shell=True)
