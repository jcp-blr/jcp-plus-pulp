...
# The path of jcp_plus_pulp_qt
_module_dir = os.path.dirname(os.path.realpath(__file__))

# The path of the jcp-plus-pulp-qt executable (when using PyInstaller)
...
ignored_filenames = ["jcp-plus-pulp-cli", "jcp-plus-pulp-client", "jcp-plus-pulp-qt", "jcp-plus-pulp-qt.desktop", "jcp-plus-pulp-qt.spec"]


def filter_modules(modules: Iterable["Module"]) -> Set["Module"]:
    # Remove things matching the pattern which is not a module
    # Like jcp-plus-pulp-qt itself, or jcp-plus-pulp-cli
...
def _discover_modules_in_directory(path: str) -> List["Module"]:
    """Look for modules in given directory path and recursively in subdirs matching jcp-plus-pulp-*"""
    modules = []
    matches = glob(os.path.join(path, "jcp-plus-pulp-*"))
    for path in matches:
        basename = os.path.basename(path)
        if is_executable(path, basename) and basename.startswith("jcp-plus-pulp-"):
...
def _discover_modules_system() -> List["Module"]:
    """Find all jcp-plus-pulp- modules in PATH"""
    ...
    for path in paths:
        ...
        for basename in ls:
            if not basename.startswith("jcp-plus-pulp-"):
                continue
...
class Module:
    ...
    def start(self, testing: bool) -> None:
        ...
        # See: https://github.com/jcp-blr/jcp-plus-pulp-server/issues/27
    ...
    def read_log(self, testing: bool) -> str:
        ...
        log_path = jcp_plus_pulp_core.log.get_latest_log_file(self.name, testing)
        if log_path:
...
class Manager:
    ...
    def discover_modules(self) -> None:
        # These should always be bundled with jcp-plus-pulp-qt
    ...
    def start(self, module_name: str) -> None:
        ...
        #       jcp-plus-pulp-qt menu since it directly calls the module's start() method.
