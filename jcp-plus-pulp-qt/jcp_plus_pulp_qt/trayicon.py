...
class TrayIcon(QSystemTrayIcon):
    def __init__(
        ...
        self.setToolTip("JCP+ PULP" + (" (testing)" if testing else ""))
    ...
    def _build_rootmenu(self) -> None:
        ...
        menu.addSeparator()
        menu.addAction(
            "Open log folder", lambda: open_dir(jcp_plus_pulp_core.dirs.get_log_dir(None))
        )
        menu.addSeparator()
        ...
        if exitIcon.availableSizes():
            menu.addAction(exitIcon, "Quit JCP+ PULP", lambda: exit(self.manager))
        else:
            menu.addAction("Quit JCP+ PULP", lambda: exit(self.manager))
...
def run(manager: Manager, testing: bool = False) -> Any:
    ...
    #   __file__ is jcp_plus_pulp_qt/trayicon.py
    #   scriptdir is ./jcp_plus_pulp_qt
    #   logodir is ./media/logo
    QtCore.QDir.addSearchPath("icons", str(scriptdir.parent / "media/logo/"))

    # When run from .app:
    #   __file__ is ./Contents/MacOS/jcp-plus-pulp-qt
    #   scriptdir is ./Contents/MacOS
    #   logodir is ./Contents/Resources/jcp_plus_pulp_qt/media/logo
    QtCore.QDir.addSearchPath(
        "icons", str(scriptdir.parent.parent / "Resources/jcp_plus_pulp_qt/media/logo/")
    )
    ...
    if not QSystemTrayIcon.isSystemTrayAvailable():
        QMessageBox.critical(
            widget,
            "Systray",
            "I couldn't detect any system tray on this system. Either get one or run the JCP+ PULP modules from the console.",
        )
        sys.exit(1)
    ...
    trayIcon.hide() # changed to .hide() from .show() by MG

    QApplication.setQuitOnLastWindowClosed(False)

    logger.info("Initialized jcp-plus-pulp-qt and trayicon successfully")
