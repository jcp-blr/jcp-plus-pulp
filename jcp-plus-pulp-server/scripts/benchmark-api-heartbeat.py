...
def benchmark():
    ds = jcp_plus_pulp_datastore.Datastore(jcp_plus_pulp_datastore.storages.PeeweeStorage, testing=True)
    api = jcp_plus_pulp_server.api.ServerAPI(ds, testing=True)
    ...
    for i in range(120):
        ...
        api.heartbeat(
            bucket_id,
            Event(timestamp=datetime.now(tz=tz.utc), data={"test": str(int(i))}),
            pulsetime=0.3,
        )
