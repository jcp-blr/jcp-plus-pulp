...
from jcp_plus_pulp_core.log import get_log_file_path
from jcp_plus_pulp_core.models import Event
from jcp_plus_pulp_query import query2
from jcp_plus_pulp_transform import heartbeat_merge
...
def get_device_id() -> str:
    path = Path(get_data_dir("jcp-plus-pulp-server")) / "device_id"
    if path.exists():
        with open(path) as f:
            return f.read()
    else:
...
class ServerAPI:
    ...
    def get_buckets(self) -> Dict[str, Dict]:
        ...
        for b in buckets:
            # TODO: Move this code to jcp-plus-pulp-core?
    ...
    def import_bucket(self, bucket_data: Any):
        ...
        # (otherwise causes weird bugs with no events seemingly imported when importing events exported from rust server, which contains IDs)
    ...
    def create_bucket(
        ...
        This is useful for watchers which are known/assumed to run locally but might not know their hostname (like jcp-plus-pulp-monitor-web).
