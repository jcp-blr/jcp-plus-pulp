...
import iso8601
from jcp_plus_pulp_core import schema
from jcp_plus_pulp_core.models import Event
from jcp_plus_pulp_query.exceptions import QueryException
from flask import (
    Blueprint,
    current_app,
    jsonify,
...
def host_header_check(f):
    """
    Protects against DNS rebinding attacks (see https://github.com/jcp-blr/jcp-plus-pulp/security/advisories/GHSA-v9fg-6g9j-h4x4)
...
# Loads event and bucket schema from JSONSchema in jcp_plus_pulp_core
    ...
    def delete(self, bucket_id):
        ...
        if not current_app.api.testing:
            if "force" not in args or args["force"] != "1":
                msg = "Deleting buckets is only permitted if jcp-plus-pulp-server is running in testing mode or if ?force=1"
...
class ExportAllResource(Resource):
    ...
    def get(self):
        ...
        filename = "jcp-plus-pulp-buckets-export.json"
...
class BucketExportResource(Resource):
    ...
    def get(self, bucket_id):
        ...
        response = make_response(json.dumps(payload))
        filename = "jcp-plus-pulp-bucket-export_{}.json".format(bucket_export["id"])
        response.headers["Content-Disposition"] = "attachment; filename={}".format(
            filename
        )
        return response
