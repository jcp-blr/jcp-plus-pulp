...
import jcp_plus_pulp_datastore
import flask.json.provider
from jcp_plus_pulp_datastore import Datastore
from flask import (
    Blueprint,
    Flask,
    current_app,
...
class AWFlask(Flask):
    def __init__(
        ...
        if storage_method is None:
            storage_method = jcp_plus_pulp_datastore.get_storage_methods()["memory"]
...
def _config_cors(cors_origins: List[str], testing: bool):
    ...
    if testing:
        # Used for development of jcp-plus-pulp-webui
        cors_origins.append("http://127.0.0.1:27180/*")

    # TODO: This could probably be more specific
    #       See https://github.com/jcp-blr/jcp-plus-pulp-server/pull/43#issuecomment-386888769
...
# Only to be called from jcp_plus_pulp_server.main function!
def _start(
    storage_method,
    host: str,
    port: int,
