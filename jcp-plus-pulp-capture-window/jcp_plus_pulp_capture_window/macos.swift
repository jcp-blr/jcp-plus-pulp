...
func logPrefix(_ level: String) -> String {
  return "\(logTimestamp()) [jcp-plus-pulp-capture-window-macos] [\(level)]"
}
...
var clientName = "jcp-plus-pulp-capture-window"
...
func start() {
  ...
  if arguments.count != 5 {
    print("Usage: jcp-plus-pulp-capture-window <url> <bucket> <hostname> <client>")
    exit(1)
  }
    ...
    if oldPayloadDifferent {
      ...
      do {
        ...
        let refreshedOldHeartbeat = Heartbeat(
          ...
          // more info: https://github.com/ActivityWatch/jcp-plus-pulp-capture-window/pull/69
          // we don't *think* this millisecond subtraction is necessary, but it may be:
          // https://github.com/ActivityWatch/jcp-plus-pulp-capture-window/pull/69#discussion_r987064282
