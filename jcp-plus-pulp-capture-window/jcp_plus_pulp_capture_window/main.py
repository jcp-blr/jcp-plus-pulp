...
def main():
    ...
    client = ActivityWatchClient(
        "jcp-plus-pulp-capture-window", host=args.host, port=args.port, testing=args.testing
    )
    ...
    logger.info("jcp-plus-pulp-capture-window started")
    ...
    with client:
        if sys.platform == "darwin" and args.strategy == "swift":
            ...
            binpath = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "jcp-plus-pulp-capture-window-macos"
            )
