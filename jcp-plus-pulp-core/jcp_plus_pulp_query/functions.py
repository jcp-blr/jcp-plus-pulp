...
from jcp_plus_pulp_transform import (
...
def q2_function(transform_func=None):
    ...
    def h(f):
        sig = signature(f)
        # If function lacks docstring, use docstring from underlying function in jcp_plus_pulp_transform
        if transform_func and transform_func.__doc__ and not f.__doc__:
            f.__doc__ = ".. note:: Documentation automatically copied from underlying function `jcp_plus_pulp_transform.{func_name}`\n\n{func_doc}".format(
