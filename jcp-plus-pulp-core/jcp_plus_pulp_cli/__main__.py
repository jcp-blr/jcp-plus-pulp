"""
The idea behind this `jcp-plus-pulp` or `jcp-plus-pulp-cli` wrapper script is to act as a collection of helper tools,
and perhaps even as a way to list and run JCP+ PULP modules on a system (a bit like jcp-plus-pulp-qt, but without the GUI).
"""
...
from jcp_plus_pulp_cli.log import find_oldest_log, print_log, LOGLEVELS
...
def qt(ctx):
    return subprocess.call(
        ["jcp-plus-pulp-qt"] + (["--testing"] if ctx.parent.params["testing"] else [])
    )
...
def directories():
    ...
    from jcp_plus_pulp_core.dirs import get_data_dir, get_config_dir, get_cache_dir, get_log_dir
...
def logs(
    ...
    from jcp_plus_pulp_core.dirs import get_log_dir
