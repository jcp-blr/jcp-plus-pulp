...
function build_zip() {
    ...
    pushd dist;
    filename="jcp-plus-pulp-${version}-${platform}-${arch}.zip"
    echo "Name of package will be: $filename"

    if [[ $platform == "windows"* ]]; then
        7z a $filename jcp-plus-pulp;
    else
        zip -r $filename jcp-plus-pulp;
    fi
    popd;
    echo "Zip built!"
}
...
function build_setup() {
    filename="jcp-plus-pulp-${version}-${platform}-${arch}-setup.exe"
    echo "Name of package will be: $filename"
    ...
    env AW_VERSION=$version_no_prefix "$innosetupdir/iscc.exe" scripts/package/jcp-plus-pulp-setup.iss
    mv dist/jcp-plus-pulp-setup.exe dist/$filename
    echo "Setup built!"
}
