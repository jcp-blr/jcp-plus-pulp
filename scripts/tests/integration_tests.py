...
#       set the PATH such that it is the "jcp-plus-pulp-server" binary.
@pytest.fixture(scope="session")
def server_process():
    logfile_stdout = tempfile.NamedTemporaryFile(delete=False)
    logfile_stderr = tempfile.NamedTemporaryFile(delete=False)

    # find the path of the "jcp-plus-pulp-server" binary and log it
    which_server = subprocess.check_output(["which", "jcp-plus-pulp-server"], text=True)
    print(f"jcp-plus-pulp-server path: {which_server}")

    server_proc = subprocess.Popen(
        ["jcp-plus-pulp-server", "--testing"], stdout=logfile_stdout, stderr=logfile_stderr
    )
...
def test_integration(server_process):
    ...
    # exit_code = pytest.main(["./jcp-plus-pulp-server/tests", "-v"])
    # if exit_code != 0:
