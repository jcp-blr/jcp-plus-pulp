...
class AFKMonitor:
    def __init__(self, args, testing=False):
        ...
        self.client = PULPMonitorClient(
            "jcp-plus-pulp-monitor-away", host=args.host, port=args.port, testing=testing
        )
    ...
    def run(self):
        logger.info("jcp-plus-pulp-monitor-away started")
        ...
        while True:
            try:
                if system in ["Darwin", "Linux"] and os.getppid() == 1:
                    ...
                    #       See: https://github.com/jcp-blr/jcp-plus-pulp-qt/issues/19#issuecomment-316741125
                    logger.info("afkmonitor stopped because parent process died")
                    break
                ...
                logger.info("jcp-plus-pulp-monitor-away stopped by keyboard interrupt")
                break
