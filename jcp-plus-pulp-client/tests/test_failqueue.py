...
def test_failqueue():
    client_name = "jcp-plus-pulp-test-client-" + str(randint(0, 10000))
    bucket_id = "test-bucket-" + str(randint(0, 10000))
    bucket_etype = "test"

    input("Make sure jcp-plus-pulp-server isn't running, then press enter > ")
    client1 = PULPMonitorClient(client_name, testing=True)
    ...
    input("Start jcp-plus-pulp-server with --testing, then press enter > ")

    client2 = PULPMonitorClient(client_name, testing=True)
