...
from jcp_plus_pulp_core import Event
...
class _Context:
    client: jcp_plus_pulp_client.PULPMonitorClient


@click.group(
    help="CLI utility for jcp-plus-pulp-client to aid in interacting with the JCP+ PULP server"
)
@click.option(
    "--host",
    default="127.0.0.1",
...
def main(ctx, testing: bool, verbose: bool, host: str, port: int):
    ctx.obj = _Context()
    ctx.obj.client = jcp_plus_pulp_client.PULPMonitorClient(
        host=host,
...
def report(
    ...
    bid_window = f"jcp-plus-pulp-monitor-window_{hostname}"
    bid_afk = f"jcp-plus-pulp-monitor-away_{hostname}"
