...
from jcp_plus_pulp_transform.heartbeats import heartbeat_merge
...
class PULPMonitorClient:
    def __init__(
        ...
        """
        A handy wrapper around the jcp-plus-pulp-server REST API. The recommended way of interacting with the server.
    ...
    def heartbeat(
        ...
        Args:
            ...
            pulsetime: The maximum amount of time in seconds since the last heartbeat to be merged with the previous heartbeat in jcp-plus-pulp-server
            queued: Use the jcp-plus-pulp-client queue feature to queue events if client loses connection with the server
...
class RequestQueue(threading.Thread):
    ...
    def __init__(self, client: PULPMonitorClient) -> None:
        ...
        data_dir = get_data_dir("jcp-plus-pulp-client")
    ...
    def _try_connect(self) -> bool:
        try:  # Try to connect
            ...
            self.connected = True
            logger.info(
                "Connection to jcp-plus-pulp-server established by {}".format(
                    self.client.client_name
                )
            )
