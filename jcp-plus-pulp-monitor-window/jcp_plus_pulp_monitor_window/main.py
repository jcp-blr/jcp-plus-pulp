...
def main():
    ...
    client = PULPMonitorClient(
        "jcp-plus-pulp-monitor-window", host=args.host, port=args.port, testing=args.testing
    )
    ...
    logger.info("jcp-plus-pulp-monitor-window started")
    ...
    with client:
        if sys.platform == "darwin" and args.strategy == "swift":
            ...
            binpath = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "jcp-plus-pulp-monitor-window-macos"
            )
...
def heartbeat_loop(client, bucket_id, poll_time, strategy, exclude_title=False):
    while True:
        if os.getppid() == 1:
            logger.info("jcp-plus-pulp-monitor-window stopped because parent process died")
            break
