...
func logPrefix(_ level: String) -> String {
  return "\(logTimestamp()) [jcp-plus-pulp-monitor-window-macos] [\(level)]"
}
...
var clientName = "jcp-plus-pulp-monitor-window"
...
func start() {
  ...
  if arguments.count != 5 {
    print("Usage: jcp-plus-pulp-monitor-window <url> <bucket> <hostname> <client>")
    exit(1)
  }
...
func sendHeartbeat(_ heartbeat: Heartbeat) {
  ...
  Task {
    if oldPayloadDifferent {
      ...
      do {
        // unlike the python jcp-plus-pulp-client library, we do not enforce a `commit_interval` and instead send the old event (which is not invalid)
        // at the current time with a pulse value equal to the time since this event was originally sent. The jcp-plus-pulp-server will then merge
        ...
        let refreshedOldHeartbeat = Heartbeat(
          ...
          // more info: https://github.com/jcp-blr/jcp-plus-pulp-monitor-window/pull/69
          // we don't *think* this millisecond subtraction is necessary, but it may be:
          // https://github.com/jcp-blr/jcp-plus-pulp-monitor-window/pull/69#discussion_r987064282
